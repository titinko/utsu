...
public class Engine {	
		...
		while (notes.hasNext()) {
			...
			if (!config.isPresent()) {
				...
				if (nextPreutter.isPresent()) {
					addSilence(
							note.getLength() - nextPreutter.get(), song, renderedNote, finalSong);
				} else {
	...
	private int getLastPitchStep(
			int totalDelta, Optional<Double> maybePreutter, double adjustedLength) {
	...
	private boolean areNotesTouching(
			Optional<SongNote> note, Voicebank voicebank, Optional<Double> nextPreutter) {
